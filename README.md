...
- Drag a region or its label to move its members together. A group with any pinned card stays in place; all three auto-arrange algorithms preserve internal group positions and arrange each group as a unit. Click a member to edit that card. **Ungroup** (Ctrl/Cmd+Shift+G, or Delete while the region is selected) removes only the boundary. Group details lets you add/remove members. Each card belongs to one group; regrouping transfers membership. Groups, styles, and membership autosave, travel with exports/imports, and support undo/redo. Deleting the last member removes the empty region.
