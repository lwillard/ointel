...
export function EdgeStyleControls({ edge, onChange }: { edge: Connector; onChange: (patch: Partial<EdgeStyle>) => void }) {
  return <div className="style-controls"><div className="section-heading"><Palette size={14} />The shape of a connection</div><p className="section-description">Give the relationship its own expression.</p>
    ...
    <Field label="Path"><select aria-label="Path" value={edge.style.path} onChange={e => onChange({ path: e.target.value as EdgeStyle['path'] })}><option value="bezier">Bézier curve</option><option value="angular">Angular</option><option value="straight">Straight</option></select></Field>
