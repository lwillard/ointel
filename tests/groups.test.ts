...
describe('card groups', () => {
  it('loads older maps and validates unique, live group membership', () => {
    ...
    expect(() => parseWorkspace({ ...w, groups: [group, { ...group, id: 'other' }] })).toThrow();
    expect(() => parseWorkspace({ ...w, groups: [{ ...group, nodeIds: ['missing'] }] })).toThrow();
  });
  it('moves members between groups and removes empty boundaries without changing cards or connections', () => {
    ...
    expect(w.groups.map(g => g.nodeIds)).toEqual([[ids[0]], [ids[1], ids[2]]]);
